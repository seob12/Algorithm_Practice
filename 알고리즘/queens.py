...
class ProducerConsumerWidget(QWidget):
    def __init__(self, buffer_capacity, queue_capacity):
        ...
        self.producer_button = QPushButton('Producer')
        self.consumer_button = QPushButton('Consumer')
    ...
    def start_producer(self):
        ...
        if not self.buffer.full.is_set():
            item = 'P' 
            
            self.buffer.push(item)
            self.update_labels()
            if len(self.buffer.buffer) == 1:
    ...
    def start_consumer(self):
        ...
        if (len(self.buffer.buffer) < 4 and len(self.buffer.buffer) > 2):
        ...
        if not self.nrempty_queue and len(self.buffer.buffer) == 0:
            self.nrempty_queue.append('E')
    ...
    def paintEvent(self, event):
        ...
        painter.fillRect(buffer_rect, QColor(255, 255, 255))  # Clear previous buffer
        ...
        painter.fillRect(queue1_rect, QColor(255, 255, 255))  # Clear previous producer queue
        ...
        painter.fillRect(queue2_rect, QColor(255, 255, 255))  # Clear previous consumer queue
        ...
        painter.fillRect(queue3_rect, QColor(255, 255, 255))  # Clear previous nrfull queue
        ...
        painter.fillRect(queue4_rect, QColor(255, 255, 255))  # Clear previous nrempty queue
